...
for (const b of definedBrands) {
  const hasDefaultModels = brandSeriesContent.includes(`${b}: [`) && 
    brandSeriesContent.slice(brandSeriesContent.indexOf(`${b}: [`)).slice(0, 3000).includes('defaultModels:');
  console.log(`${b}: has defaultModels = ${hasDefaultModels}`);
}
