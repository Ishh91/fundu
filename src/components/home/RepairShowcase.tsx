      ...
      <div className="rounded-3xl border border-gray-200 bg-white p-6 sm:p-8 md:p-10 shadow-sm">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <Link
              to="/repair"
              className="inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-[#344257] to-[#5D6A82] hover:brightness-110 px-5 py-3 text-sm font-bold text-white shadow-md transition"
            >
            ...
            return (
              <div
                ...
                className={`cursor-pointer rounded-2xl border p-4 transition-all duration-200 flex flex-col justify-between ${
                  isSelected
                    ? 'border-2 border-[#344257] bg-[#F0F0F5]/80 shadow-md'
                    : 'border-[#C0C8D8]/60 bg-[#F7F7FA] hover:border-[#6A859F] hover:bg-white'
                }`}
              >
                <div>
                  <div className="flex items-center justify-between">
                    ...
                    <span className="rounded-full bg-white border border-[#C0C8D8]/70 px-2 py-0.5 text-[10px] font-bold text-[#47576E]">
                      {srv.estTime}
                    </span>
                  </div>

                  <h3 className="mt-3 text-sm font-bold text-[#344257]">{srv.title}</h3>
                  <p className="mt-1 text-[11px] leading-relaxed text-[#8A9AAF]">{srv.desc}</p>
                </div>

                <div className="mt-4 pt-3 border-t border-gray-100 flex items-center justify-between">
                  <div>
                    <span className="text-[10px] font-semibold text-gray-400 block">Starts from</span>
                    <span className="text-sm font-black text-[#344257]">{formatINR(srv.priceFrom)}</span>
                  </div>
                  <Link
                    to={`/repair?issue=${srv.id}`}
                    className="rounded-lg bg-gradient-to-r from-[#344257] to-[#5D6A82] px-3 py-1 text-xs font-bold text-white hover:brightness-110 shadow-xs"
                  >
                    Book
                  </Link>
                </div>
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-2 md:grid-cols-4 rounded-2xl bg-[#F7F7FA] p-4 border border-[#C0C8D8]/60">
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-xs font-bold text-[#344257]">30-Min Instant Doorstep Repair</span>
          </div>
          <div className="flex items-center gap-2.5">
            <ShieldCheck className="h-4 w-4 text-[#6A859F] shrink-0" />
            <span className="text-xs font-bold text-[#344257]">Up to 6 Months Replacement Warranty</span>
          </div>
          <div className="flex items-center gap-2.5">
            <CheckCircle2 className="h-4 w-4 text-[#6A859F] shrink-0" />
            <span className="text-xs font-bold text-[#344257]">100% Genuine Tested Spare Parts</span>
          </div>
          <div className="flex items-center gap-2.5">
